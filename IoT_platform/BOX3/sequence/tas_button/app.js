...
const button = new Gpio('5', 'in', 'falling', {debounceTimeout: 30});   // GPIO16 (pin): button input
...
button.watch((err, value) => {
    ...
    if (tas_state == 'upload') {
        for (var i = 0; i < upload_arr.length; i++) {
            if (upload_arr[i].ctname == 'button_BOX3') {
                var cin = {ctname: upload_arr[i].ctname, con: 'BOX3'};
