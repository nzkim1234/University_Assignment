...
def get_origin_data(num):
    ...
    try:
        ...
        print("helfewoihfewoiheowfih")
        return False

def get_box_number(num):
    url = "http://180.83.19.43:7579/Mobius/test/box" + str(num) + "?fu=2&la=3&ty=4&rcn=4"
...
def del_origin_data(num, cin_origin_data_rn):
    url = "http://180.83.19.43:7579/Mobius/test/" + str(num) + "/origin_data?fu=2&la=3&ty=4&rcn=4"
  ...
  for d in decoded: 
    ...
    for i in r:
            if get_origin_data(i) == str(int(barcode_data) // int(key)):
                ...
                for f in range(3):
                    a = f + 1
                    if get_box_number(a) == str(i):
                        print("And " + str(i) + " is in Box" + str(a))
                        # get_servo(a)
                        # del_servo(a, cin_servo_rn)
                        # post_servo(a)
                        del_origin_data(i, cin_origin_data_rn)
  cv2.imshow('img', img)

  key = cv2.waitKey(1)
  if key == ord('q'): #q키를 누르면 이미지 창이 꺼진다
    break
  elif key == ord('s'): #s키를 누르면 캡쳐된다.
    i += 1
